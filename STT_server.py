...
@app.route('/text_only_test/<string:input_text>')
def text_only_test(input_text):
    #input_text = request.args.get('input_text')
    translated_text = main_module(input_text)
    return jsonify(text=input_text, text2=translated_text)

@app.route('/test')
def test_output():
    return jsonify(text="테스트 화면 출력")   
